//
// Created by Juan Rodriguez on 2025-03-01.
//
...
static inline void JoinThreads(pthread_t *threads,int N, int M, double *ax, double *ay);
static inline void CreateThreads(pthread_t *threads, thread_container *thread_containers, int M);
...
int main(int argc, char *argv[]){
    ...
    for (n = 0; n < n_steps; n++){
        ...
        CreateThreads(threads, thread_containers, M);
        JoinThreads(threads, N, M, ax, ay);
