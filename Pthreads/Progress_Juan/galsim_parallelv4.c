...
int main(int argc, char *argv[]) {
    ...
    GET_TIME(start);
    for (n = 0; n < n_steps; n++) {
        // Iteratively create and join threads -> Increases thread creation overhead -> May affect performance
        for (int i = 0; i < M; i++) {
        ...
        // but N is comparatively small to N(N-1)/2 for larger N
        for (int i = 0; i < N; i++) {
            u[i] += G * (ax[i] * dt);
            x[i] += u[i] * dt;
            v[i] += G * (ay[i] * dt);
...
void *compute_accelerations(void *arg) {
    ...
    double *a = calloc(2 * N, sizeof(double)); // Could define allocation in main
    double *ax = a, *ay = a + N;
    int i, j;
    double m_j, x_i, y_i;
